...
const getData = async () => {
  loading.value = true;
  const { data: car_brand, error } = await supabase.from("car_brands").select("*").eq("id", props.id).single();
  if (error) {
    $toast.error(error.message);
  } else {
    formData.value = car_brand;
  }
  loading.value = false;
};
...
const onSubmit = async () => {
  ...
  if (validate.valid) {
    ...
    if (props.actionType == "add") {
      const { data, error } = await supabase.from("car_brands").insert(formData.value);
      error ? $toast.error(error.message) : ($toast.success("ทำรายการสำเร็จ"), (dialog.value = false), emit("success"));
    } else {
      const { data, error } = await supabase.from("car_brands").update(formData.value).eq("id", formData.value.id);
      error
        ? $toast.error(error.message)
...
const deleteItem = async () => {
  ...
  const { data, error } = await supabase.from("car_brands").delete().eq("id", id.value);
