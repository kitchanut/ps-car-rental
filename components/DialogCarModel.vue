  ...
  <v-dialog v-model="dialog" fullscreen :scrim="false" transition="dialog-bottom-transition">
    <template v-slot:activator="{ props: activatorProps }">
      ...
      <v-btn
        ...
        :color="props.status == 'เปิดใช้งาน' ? 'primary' : 'grey'"
        dark
        v-bind="activatorProps"
        density="comfortable"
        variant="text"
        style="min-width: 0px; padding: 0px; font-size: 1rem"
      >
    ...
    <v-card>
      <v-form ref="form" lazy-validation @submit.prevent="onSubmit()">
        ...
        <v-container>
          <v-card :loading="loading" :disabled="loading" variant="outlined" style="border: thin solid #ddd !important">
            <v-card-text>
              ...
              <v-select
                ...
                v-model="formData.car_model_status"
                density="comfortable"
                label="สถานะ"
                hide-details
                :rules="[(value) => !!value || 'Required.']"
          ...
          <v-btn
            v-if="props.actionType == 'edit' && formData.car_model_status == 'ระงับการใช้งาน'"
            class="mt-5"
            color="error"
            variant="tonal"
            block
    ...
    nextTick(() => {
      ...
      nextTick(() => {
        if (props.actionType == "add") {
          ...
          formData.value.car_brand_id = props.car_brand_id;
          loading.value = false;
        } else {
