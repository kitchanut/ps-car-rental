      ...
      <v-table density="compact">
        <thead>
          <tr>
            ...
            <th class="text-left px-2">รับรถ|คืนรถ</th>
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-2">
              ...
              <div style="color: grey">{{ $dayjs(item.return_date).format("YYYY-MM-DD") }}</div>
            </td>
            <td class="px-2">
            ...
            <td align="end" class="px-2">
              <div style="color: green">{{ Number(item.sumExpense).toLocaleString() }}</div>
            </td>
          </tr>
          <tr v-else>
            <td colspan="5">
...
const getData = async () => {
  loading.value = true;
  let queryString = `?query_period_by=return_date&period=${period.value}&status=คืนมัดจำ&include=account_transactions`;
  const response = await useApiBookings().index(queryString);
  console.log(response.data);
  data.value = response.data;
  data.value.map((item) => {
    item.sumExpense = item.account_transactions.reduce((acc, cur) => acc + cur.transaction_amount, 0);
  });
  loading.value = false;
};
getData();
