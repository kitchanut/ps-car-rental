  ...
  <div class="pa-3">
    ...
    >
      <v-card-title class="d-flex justify-space-between align-center">
        <div>{{ $dayjs(item.pickup_date).format("YYYY-MM-DD HH:mm") }}</div>
        <span style="font-size: 1rem" :style="{ color: colorBar(item.booking_status) }">{{ item.booking_number }}</span>
      </v-card-title>
      <v-card-text>
        <v-row no-gutters>
          <v-col cols="4">สถานที่:</v-col>
          <v-col class="text-right"> {{ item.pickup_location }} | {{ item.pickup_branch.branch_name }} </v-col>
        </v-row>
        <v-row no-gutters>
      ...
      <v-table density="compact">
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-2">
              <div>{{ $dayjs(item.pickup_date).format("YYYY-MM-DD") }}</div>
              <div style="color: grey">{{ $dayjs(item.pickup_date).format("HH:mm") }}</div>
            </td>
...
const pickup_date = ref(new Date().toISOString().substr(0, 10));
const data = ref([]);
const getData = async () => {
  loading.value = true;
  let startDate = dayjs(pickup_date.value).startOf("day").format("YYYY-MM-DD");
  let endDate = dayjs(pickup_date.value).endOf("day").format("YYYY-MM-DD 23:59");
  let query = supabase
    .from("bookings")
    .select("*,cars(*,car_models(*)),customers(*),pickup_branch:bookings_pickup_branch_id_fkey(*)")
    .eq("booking_status", "รับเงินจอง")
    .gte("pickup_date", startDate)
    .lte("pickup_date", endDate)
    .order("pickup_date");
  if (branch_id.value != null) {
    query = query.eq("pickup_branch_id", branch_id.value);
  }
